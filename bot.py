...
ANTHROPIC_KEY  = config.get("GEMINI_API_KEY", "")
...
async def cmd_testkey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔑 Testing your Gemini API key…")
...
async def run_pipeline(bot, cid: int, ctx: ContextTypes.DEFAULT_TYPE, msg_id: int):
    ...
    try:
        ...
        pipeline = MangaPipeline(
            ...
            settings=s,
            anthropic_key=ANTHROPIC_KEY,  # now holds GEMINI_API_KEY
