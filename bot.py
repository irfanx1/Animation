...
def db_init():
    ...
    """)
    # Migrate existing DBs — add bg_blur if missing
    try:
        con.execute("ALTER TABLE users ADD COLUMN bg_blur INTEGER DEFAULT 0")
        con.commit()
...
CYCLES = {
    ...
    "quality":     ["sd","hd","4k"],
    "bg_blur":     [0, 1, 2],  # 0=off, 1=blur, 2=blur+zoom
}
LABELS = {
...
def lbl(key:str, val) -> str:
    return LABELS.get(key,{}).get(val, str(val).title())

def next_val(current, key:str):
    lst = CYCLES[key]
...
def kb_settings(s:dict) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("🌫️ BG Mode",         callback_data="info:bg"),
         InlineKeyboardButton(lbl("bg_blur",s.get("bg_blur",0)), callback_data="cycle:bg_blur")],
...
def settings_text(s:dict) -> str:
    return (
        ...
        f"🌫️ BG Mode    : {lbl('bg_blur', s.get('bg_blur',0))}\n\n"
...
━━━━ 🌫️ BG MODE ━━━━
This controls how the manga page fills the video frame:

🖼️ *Crop Fill* — Zooms into the page to fill the entire screen. 
No black bars. Best when you want full-screen impact.

🌫️ *Blur BG* — Shows the FULL manga page in the centre. 
Blurred version of the same page fills the background. 
Looks like YouTube Shorts / Instagram Reels style.

🌫️✨ *Blur+Zoom* — Same as Blur BG but the page slowly 
and gently zooms in while the blurred bg stays still. 
Most cinematic-looking option. RECOMMENDED.
...
INFO_MSGS = {
    ...
    "bg":      "🌫️ BG Mode:\n🖼️ Crop Fill — zooms in, fills frame edge to edge\n🌫️ Blur BG — shows FULL manga page, blurred bg behind it\n🌫️✨ Blur+Zoom — full page + blurred bg + slow gentle zoom",
}
...
async def on_callback(update:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cycle:"):
        ...
        db_set(cid, lang="en", voice="calm", style="cinematic",
               color_grade="vivid", subtitles=1, speed="normal", quality="hd", bg_blur=0)
        s = db_get(cid)
        await q.edit_message_text(
