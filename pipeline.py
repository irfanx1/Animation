...
class MangaPipeline:
    ...
    # ── Step 3: AI Script (Groq — FREE, no credit card needed) ─────────
    async def _ai_script(self, images: list[Path]) -> list[dict]:
        await self.upd(30, "Groq AI is reading your manga…")
        ...
        if not key:
            raise ValueError(
                ...
                "Sign up with Google — no credit card needed!"
            )
        if not key.startswith("gsk_"):
            raise ValueError(
                f"GROQ_API_KEY looks wrong (got: {key[:12]}...)\n"
                "It must start with 'gsk_'\n"
                "Get a fresh key at console.groq.com"
            )

        # Groq supports vision via llama-4-scout
        # Build messages with base64 images
        image_contents = []
        for p in images[:8]:   # Groq vision: up to 8 images
            with open(p, "rb") as f:
        ...
        image_contents.append({
            ...
            "text": (
                ...
                "Analyse these manga page images. "
                "Return ONLY a valid JSON array, no markdown, no extra text. "
                "One object per image:\n"
                '[{"panel":1,"narration":"2-3 vivid spoken sentences","subtitle":"max 10 words","emotion":"action"}]\n'
                f"Return exactly {len(images[:8])} objects, one per image."
            )
        })
        ...
        for attempt in range(1, 5):
            ...
            if resp.status_code not in RETRYABLE:
                break
            wait = attempt * 8
            await self.upd(30, f"Groq busy — retrying in {wait}s… ({attempt}/4)")
            await asyncio.sleep(wait)

        if resp.status_code == 401:
            raise ValueError(
                "Groq API key rejected (401)!\n"
                "Get a fresh key at console.groq.com → API Keys"
            )
        elif resp.status_code == 429:
            raise ValueError("Groq rate limit hit. Wait 1 minute and try again.")
        ...
        try:
            ...
            raise ValueError(f"Unexpected Groq response: {resp.text[:300]}")

        raw = raw.lstrip("```json").lstrip("```").rstrip("```").strip()
        panels = json.loads(raw)

        # If AI returned fewer panels than images, pad with generic ones
        while len(panels) < len(images[:8]):
            i = len(panels) + 1
            panels.append({
                "panel": i,
                "narration": f"The story continues on panel {i}, filled with tension and drama.",
                "subtitle": f"Panel {i} — the story unfolds",
                "emotion": "calm"
            })
        return panels
