...
def build_scroll_blur_frame(img, tw, th, progress, blur_radius=22,
                             brightness=0.38, zoom_amount=0.0):
    """
    SCROLL + BLUR BG MODE:
    - Manga page centred with correct aspect ratio
    - Left & right blurred columns from same image
    - Camera scrolls page top-to-bottom
    - Looks exactly like the screenshot you showed but FULL QUALITY
    """
    img = img.convert("RGB")
    sw, sh = img.size
    p  = ease_in_out(progress)

    # Determine page display size — fit to height, keep aspect ratio
    scale = th / sh
    nw    = int(sw * scale)
    nh    = th
    ...
    # Apply gentle zoom
    if zoom_amount > 0:
        extra = 1.0 + zoom_amount * 0.25 * p
        nw2   = int(nw * extra)
        nh2   = int(nh * extra)
        fg    = img.resize((nw2, nh2), Image.BILINEAR)
        # Scroll vertically if zoomed page is taller
        if nh2 > th:
            scroll_y = int((nh2 - th) * p)
            fg = fg.crop((0, scroll_y, nw2, scroll_y + th))
            nh2 = th
        # Crop width to nw
        if nw2 > nw:
            ox = (nw2 - nw) // 2
            fg = fg.crop((ox, 0, ox + nw, nh2))
        nw_final, nh_final = nw, nh2
    else:
        # Scroll: scale to full height, scroll horizontally if wide
        # OR scale to full width and scroll vertically
        # Choose: if portrait page, fit height. If landscape, fit width+scroll
        if sh > sw:
            # Portrait manga page — scale to fit height, scroll if needed
            scale_h = th / sh
            nw_fit  = int(sw * scale_h)
            nh_fit  = th
            page_rs = img.resize((nw_fit, nh_fit), Image.BILINEAR)

            # If page is narrower than video, just centre it (blur bg handles sides)
            fg       = page_rs
            nw_final = nw_fit
            nh_final = nh_fit
        else:
            # Landscape — scale to full width, scroll top to bottom
            scale_w = tw / sw
            nw_fit  = tw
            nh_fit  = int(sh * scale_w)
            page_rs = img.resize((nw_fit, nh_fit), Image.BILINEAR)

            if nh_fit > th:
                scroll_y = int((nh_fit - th) * p)
                fg = page_rs.crop((0, scroll_y, nw_fit, scroll_y + th))
                nh_fit = th
            else:
                fg = page_rs
            nw_final = nw_fit
    ...
    # ── Paste page centred over bg ────────────────────────────
    result = bg.copy()
    x = (tw - nw_final) // 2
    y = (th - nh_final) // 2
    result.paste(fg, (x, max(0, y)))

    # Clean thin border around page
    draw = ImageDraw.Draw(result)
    draw.rectangle([x-2, max(0,y)-2, x+nw_final+2, min(th,y+nh_final)+2],
