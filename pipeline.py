...
# ─────────────────────────────────────────────
#  CORE FRAME BUILDER
#  Manga page fits full height, centred,
#  light-grey blurred bg on sides — no borders
# ─────────────────────────────────────────────
def build_frame(img: Image.Image,
    ...
    """
    THE renderer. Matches the reference screenshot exactly:
    • Manga page scaled to fill full height
    • Blurred version of same page fills left/right sides
    • Light grey bg (brightness ~0.82) — NOT dark
    • Subtle zoom while camera scrolls down
    • Zero borders / zero padding lines
    """
    img = img.convert("RGB")
    ...
    # ── Foreground: scale page to fit height ──────────────
    ...
    # Vertical scroll: if page is taller than frame, scroll down
    if nh > frame_h:
...
# ─────────────────────────────────────────────
#  SUBTITLE RENDERER  — small, clean, modern
# ─────────────────────────────────────────────
_FONT_CACHE: dict = {}
...
def draw_subtitle(frame: Image.Image, text: str,
    ...
    """Small, clean, Netflix-style subtitle bar. No clutter."""
    ...
    # Thin dark bar — full width, minimal height
    bar_a = int(160 * alpha)
    draw.rectangle([0, y0 - 8, fw, y0 + total_h + 8],
                   fill=(0, 0, 0, bar_a))

    # White text, tight shadow
    txt_a = int(255 * alpha)
...
# ─────────────────────────────────────────────
#  STYLE FX  (vignette, grain, glow, sharpen)
# ─────────────────────────────────────────────
...
def apply_style(img: Image.Image, anim_style: str,
    ...
    # Flash at very start for action panels
...
class MangaPipeline:
    ...
    def __init__(self, bot, chat_id, update_progress,
        ...
        # Portrait 9:16 — correct for manga on phones
        q          = settings.get("quality", "hd")
        dims       = QUALITY_DIMS.get(q, (1080, 1920))
        self.fw    = dims[0]   # frame width
        self.fh    = dims[1]   # frame height
    ...
    # ──────────────────────────────────────────
    #  3. AI NARRATION  (Groq, batched by 4)
    # ──────────────────────────────────────────
    async def _narrate(self, images: list[Path]) -> list[dict]:
        ...
        for b_idx, batch in enumerate(batches):
            ...
            # Compress images small for API (avoid 413 errors)
            parts = []
            for p in batch:
            ...
            # Pad with fallback if AI returned fewer than batch size
    ...
    def _render_clip(self, src_img: Image.Image, audio: Path,
        ...
        # Pre-grade the image once
        src = grade(src_img.convert("RGB"), clr_grade)

        # Render frames directory
