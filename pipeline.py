...
# - Portrait 9:16 frame
# - Manga centred, light-grey blurred sides
# - NO border, NO dark bg
# - Subtle zoom while scrolling
# ═══════════════════════════════════════════════
def build_frame(img, tw, th, progress,
                blur_radius=18, brightness=0.82,
                zoom_amount=0.05, zoom_in=True,
                parallax=False, ease_fn=ease_in_out):
    """
    THE main renderer — exactly like the reference screenshot.
    bg brightness=0.82 = light grey (not dark).
    No borders. No padding. Clean.
    """
    img = img.convert("RGB")
    sw, sh = img.size
    p = ease_in_out(progress)

    # ── Background: blurred version stretched to fill frame ──
    # Use brightness ~0.80-0.85 so bg is LIGHT GREY like screenshot
    bg = img.resize((tw, th), Image.BILINEAR)
    bg = bg.filter(ImageFilter.GaussianBlur(radius=max(blur_radius, 8)))
    bg = ImageEnhance.Brightness(bg).enhance(brightness)

    # Optional parallax: bg drifts slightly opposite to page
    if parallax and zoom_amount > 0:
        pan = int(tw * 0.015 * p) * (1 if zoom_in else -1)
        bg_arr = np.array(bg)
        if pan != 0:
            bg_arr = np.roll(bg_arr, pan, axis=1)
            if pan > 0: bg_arr[:, :pan] = bg_arr[:, pan:pan+1]
            else:       bg_arr[:, pan:] = bg_arr[:, pan-1:pan]
        bg = Image.fromarray(bg_arr)

    # ── Foreground: scale manga to fit height, keep aspect ───
    # This matches screenshot: manga fills ~65% of width,
    # full height, blurred grey on left & right
    scale = th / sh          # fit to frame height
    nw    = int(sw * scale)
    nh    = th

    # Apply zoom (slow scroll down + gentle zoom in)
    if zoom_amount > 0:
        z     = 1.0 + zoom_amount * p if zoom_in else 1.0 + zoom_amount * (1-p)
        nw    = int(nw * z)
        nh    = int(nh * z)

    fg = img.resize((nw, nh), Image.BILINEAR)

    # Scroll: pan vertically through the page (top → bottom)
    if nh > th:
        scroll_y = int((nh - th) * p)
        fg = fg.crop((0, scroll_y, min(nw, tw + (nw-int(sw*th/sh))//2 * 2),
                      scroll_y + th))
        # Re-centre after crop
        fg_w = fg.width
        fg_h = th
    else:
        fg_w = nw
    ...
    # Centre on background — NO border, NO padding
    x = (tw - fg_w) // 2
    y = (th - fg_h) // 2 if fg_h < th else 0

    result = bg.copy()
    result.paste(fg, (max(0, x), max(0, y)))
...
def build_blur_bg(img, tw, th, blur_radius, brightness, progress=0.0,
                  ...
                  parallax=False):
    return build_frame(img, tw, th, progress,
                       blur_radius=blur_radius, brightness=brightness,
                       zoom_amount=zoom_amount, zoom_in=zoom_in,
                       parallax=parallax, ease_fn=ease_fn)


# ═══════════════════════════════════════════════
# VERTICAL SCROLL RENDERER
# The manga page fills full width, camera scrolls top→bottom
# exactly like reading manga, while narrator explains
# ═══════════════════════════════════════════════
def build_scroll_frame(img, tw, th, progress, ease_fn=ease_in_out,
                       blur_sides=True, blur_radius=20, brightness=0.38,
                       zoom_amount=0.0):
    """
    SCROLL MODE: Page fills full width, camera scrolls vertically.
    - Full manga page width = video width (no crop, no bars)
    - Camera moves top-to-bottom smoothly during audio duration
    - Optional: blurred left/right sides if page is portrait
    - Optional: subtle zoom while scrolling
    """
    img = img.convert("RGB")
    sw, sh = img.size
    p = ease_in_out(progress)  # always smooth scroll

    # Scale page to fit full width of video
    scale = tw / sw
    nw    = tw
    nh    = int(sh * scale)

    page  = img.resize((nw, nh), Image.BILINEAR)

    # Apply subtle zoom on top of scroll
    if zoom_amount > 0:
        extra = 1.0 + zoom_amount * 0.3  # keep zoom very gentle in scroll mode
        znw   = int(nw * extra)
        znh   = int(nh * extra)
        page  = page.resize((znw, znh), Image.BILINEAR)
        # Centre horizontally
        off_x = (znw - nw) // 2
        page  = page.crop((off_x, 0, off_x + nw, znh))
        nh    = znh

    # Scroll: if page is taller than frame, scroll through it
    if nh > th:
        max_scroll = nh - th
        scroll_y   = int(max_scroll * p)
        frame      = page.crop((0, scroll_y, nw, scroll_y + th))
    else:
        # Page fits in frame: centre it vertically with blurred bg
        frame = Image.new("RGB", (tw, th), (0, 0, 0))
        y_off = (th - nh) // 2
        frame.paste(page, (0, y_off))
        return frame

    # If page is narrower than video (unlikely but handle it)
    if nw < tw:
        canvas = Image.new("RGB", (tw, th), (20, 20, 20))
        canvas.paste(frame, ((tw - nw) // 2, 0))
        frame = canvas

    return frame


def build_scroll_blur_frame(img, tw, th, progress, blur_radius=18,
                             brightness=0.82, zoom_amount=0.05):
    """
    Scroll+Blur — exactly like the reference screenshot:
    - Light grey blurred left & right sides (NOT dark)
    - Manga page centred, correct aspect ratio
    - Slowly scrolls from top to bottom of page
    - NO border whatsoever
    """
    # Delegate to the main build_frame function
    return build_frame(img, tw, th, progress,
        ...
        if is_action and s.get("action_clips",1) and style in ("manga","dramatic","cinematic"):
            ...
            for fi in range(n_frames):
                ...
                if bg_val==0:
                    ...
                    # SCROLL MODE — full width, camera scrolls top to bottom
                    frame=build_scroll_frame(base,tw,th,progress,ease_fn,
                                             blur_radius=blur_r,brightness=blur_b,
                                             zoom_amount=zoom_amt)
                    frame=apply_grade(frame,grade)
                    frame=apply_style_fx(frame,style,progress,emotion)
                else:
                    # SCROLL + BLUR BG — centred page, blurred sides, scrolling
                    frame=build_scroll_blur_frame(base,tw,th,progress,
                                                  blur_radius=blur_r,
                                                  brightness=blur_b,
                                                  zoom_amount=zoom_amt)
                    frame=apply_style_fx(frame,style,progress,emotion)
