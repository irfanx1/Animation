...
class MangaPipeline:
    ...
    async def _ai_script(self, images: list[Path]) -> list[dict]:
        ...
        if not key:
            raise ValueError(
                "ANTHROPIC_API_KEY is empty in config.json!\n"
                "Get your key at: https://console.anthropic.com"
            )
        if not key.startswith("sk-ant-"):
            raise ValueError(
                f"ANTHROPIC_API_KEY looks wrong (got: {key[:12]}...)\n"
                "It must start with 'sk-ant-'. Copy it fresh from console.anthropic.com"
            )
        ...
        payload = {
            "model": "claude-haiku-4-5-20251001",   # faster + cheaper for narration
            "max_tokens": 2048,
            "system": system,
        ...
        # Auto-retry for 502/503/529 temporary server errors
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if resp.status_code not in RETRYABLE:
                break
            wait = attempt * 8
            await self.upd(30, f"Anthropic server busy — retrying in {wait}s… (attempt {attempt}/{MAX_RETRIES})")
        ...
        if resp.status_code == 401:
            raise ValueError(
                ...
                "Fix: go to console.anthropic.com → add billing + fresh API key"
            )
        elif resp.status_code == 403:
            raise ValueError("API key has no access. Add billing at console.anthropic.com/billing")
        elif resp.status_code == 429:
            raise ValueError("Rate limit hit. Wait a minute and try again.")
        elif resp.status_code in RETRYABLE:
            raise ValueError(
                f"Anthropic servers temporarily down ({resp.status_code}). "
