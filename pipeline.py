...
def crop_to_fill(img:Image.Image, tw:int, th:int) -> Image.Image:
    """Scale image so it covers target, then centre-crop. ZERO black bars."""
    img   = img.convert("RGB")
    sw,sh = img.size
...
def render_frame(base:Image.Image, progress:float, style:str,
    ...
    if style=="cinematic":
        scale = (1.0+0.09*p) if zoom_in else (1.09-0.09*p)
        px    = int(w*0.025*p)*(1 if zoom_in else -1)
        py    = int(h*0.018*p)
    elif style=="manga":
        scale = 1.0+0.14*min(p*2,1.0)
        px=py=0
    elif style=="noir":
        scale = 1.0+0.07*p
        px    = int(w*0.035*p)
        ...
        scale = (1.0+0.15*p) if zoom_in else (1.15-0.15*p)
        px    = int(w*0.04*p)*(1 if zoom_in else -1)
        py    = int(h*0.02*p)
    else:
        scale=1.0+0.07*p; px=py=0
    ...
    if style in ("noir","dramatic"):
        img = add_vignette(img, 0.55 if style=="noir" else 0.35)
    elif style=="retro":
        arr   = np.array(img,dtype=np.int16)
        grain = np.random.randint(-14,14,arr.shape,dtype=np.int16)
        ...
        img   = ImageChops.screen(img, ImageEnhance.Brightness(glow).enhance(0.35))
    elif style=="manga":
...
def burn_subtitle(frame:Image.Image, text:str, alpha:float,
    ...
    y0      = h-total_h-40
    bg_a    = int(210*alpha)
    draw.rounded_rectangle([14,y0-12,w-14,y0+total_h+8], radius=16, fill=(0,0,0,bg_a))
    txt_a = int(255*alpha)
    colors = {
        "dramatic":(255,215,0,txt_a),
        "noir":(210,210,210,txt_a),
        "retro":(255,195,100,txt_a),
        "anime":(140,215,255,txt_a),
    }
    color = colors.get(style,(255,255,255,txt_a))
    for i,line in enumerate(lines):
        bbox = draw.textbbox((0,0), line, font=fnt)
        lw   = bbox[2]-bbox[0]
        x    = (w-lw)//2
        y    = y0+i*lh
        draw.text((x+2,y+2), line, font=fnt, fill=(0,0,0,int(txt_a*0.85)))
...
class MangaPipeline:
    ...
    def _render_panel(self, base:Image.Image, audio:Path,
        ...
        for fi in range(n_frames):
            ...
            frame    = render_frame(base,prog,style,zoom_in,tw,th,subtitle,alpha)
